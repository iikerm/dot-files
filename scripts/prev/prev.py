...
modules_path = f"/home/.scripts"    # Add your own path here
sys.path.append(modules_path)   #So it can import things from .py files in .scripts dir
import stylelib as sts

scriptPath = f"{modules_path}/prev/"
...
helpText = f"""\t{sts.bold_code}{sts.uline_code}Prev tool help:{sts.end_style_code}

        {sts.bold_code}Usage:{sts.end_style_code}
        ...
        {sts.bold_code}Arguments:{sts.end_style_code}
        ...
        {sts.bold_code}Extra arguments that can be used with text files:{sts.end_style_code}
...
def listDirs(dirToList=os.getcwd(), spaces=0):
    print("\n"*spaces)
    ...
    # dir_size = os.stat(os.getcwd()).st_size
    # dir_size = round(dir_size * 1000*1000, 4)
    # \t\t{styles.bold_code}({dir_size} MB){styles.end_style_code}
    # Doesnt work for folders, should recurse into each individual folder

    print(f"{sts.uline_code}󱞊    {sts.bold_code}{dirToList}{sts.end_style_code}")       # 󱞊 is the nerdfonts icon for a folder with an eye
...
if len(sys.argv) <= 1:
    # print(styles.formatAsError(" | Invalid syntax, please include options"))
...
if not os.path.exists(fname):
    print(sts.formatAsError(" | Given file does not exist (" + f"\033[97m'{fname}'{sts.end_style_code}" + sts.formatAsError(")")))
    raise SystemExit
...
if ext != fname and imgTypes.__contains__(ext):
    ...
    imgData = [f" {sts.bold_code}File name:\t\t{sts.end_style_code}{fname}", 
               f" {sts.bold_code}File size:\t\t{sts.end_style_code}{round((file_stats.st_size)/(1000*1000), 4)} MB", 
               f"󱇣 {sts.bold_code}Modification date:\t{sts.end_style_code}{dt.datetime.fromtimestamp(os.path.getmtime(os.getcwd()+'/'+fname))}", 
               f"󰙴 {sts.bold_code}Creation date:\t{sts.end_style_code}{file_ctime}"]
    ...
    try:
        ...
        print(sts.formatAsError("| Unable to preview this file type (") + f"\033[97m'{ext}'{sts.end_style_code}" + sts.formatAsError(")"))
        raise SystemExit
    ...
    for arg in args:
        if arg.__contains__(validArguments["show_lines1"]):
            ...
            linesToShow = checkInt(linesToShow, sts.formatAsError("| Invalid number of lines given (") + f"\033[97m'{linesToShow}'{sts.end_style_code}" + sts.formatAsError(")"))
    ...
    for arg in args:
        if arg.__contains__(validArguments["see_from1"]):
            ...
            startingPos = checkInt(startingPos, sts.formatAsError("| Invalid position to read from (") + f"\033[97m'{startingPos}'{sts.end_style_code}" + sts.formatAsError(")"))
    ...
    if startingPos >= len(fileContent):
        print(sts.formatAsError("| Position to read from is bigger than file length (") + f"\033[97m{startingPos+1}{sts.end_style_code}" + sts.formatAsError(")"))
        raise SystemExit
    elif startingPos < 0:
        print(sts.formatAsError("| Invalid position to read from (") + f"\033[97m{startingPos+1}{sts.end_style_code}" + sts.formatAsError(")"))
        raise SystemExit
    else:
